...
data class IkoroPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = IkoroPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.ikoro.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): IkoroPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: IkoroPacket): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): IkoroPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): IkoroPacket? {
        try {
            ...
            if (payloadLength > com.ikoro.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.ikoro.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
