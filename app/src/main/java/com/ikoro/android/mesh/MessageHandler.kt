...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.ikoro.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.ikoro.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.ikoro.android.model.PrivateMessagePacket.decode(noisePayload.data)
                    if (privateMessage != null) {
                        ...
                        // Create IkoroMessage - preserve source packet timestamp
                ...
                com.ikoro.android.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.ikoro.android.model.IkoroFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.ikoro.android.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = IkoroMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.ikoro.android.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.ikoro.android.model.NoisePayloadType.DELIVERED -> {
                ...
                com.ikoro.android.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                com.ikoro.android.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                ...
                com.ikoro.android.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.ikoro.android.model.NoisePayload(
                type = com.ikoro.android.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.ikoro.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.ikoro.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
        ...
        try {
            val neighborsOrNull = com.ikoro.android.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = IkoroPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.ikoro.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.ikoro.android.protocol.MessageType.fromValue(packet.type) == com.ikoro.android.protocol.MessageType.FILE_TRANSFER
            val file = com.ikoro.android.model.IkoroFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
     ...
     */
    private suspend fun handlePrivateMessage(packet: IkoroPacket, peerID: String) {
        try {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.ikoro.android.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.ikoro.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.ikoro.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.ikoro.android.model.IkoroMessage(
                    sender = "system",
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: IkoroPacket)
    fun relayPacket(routed: RoutedPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: IkoroPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: IkoroMessage)
