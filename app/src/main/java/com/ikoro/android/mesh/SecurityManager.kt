...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = com.ikoro.android.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
        private const val CLEANUP_INTERVAL = com.ikoro.android.util.AppConstants.Security.CLEANUP_INTERVAL_MS // 5 minutes
        private const val MAX_PROCESSED_MESSAGES = com.ikoro.android.util.AppConstants.Security.MAX_PROCESSED_MESSAGES
        private const val MAX_PROCESSED_KEY_EXCHANGES = com.ikoro.android.util.AppConstants.Security.MAX_PROCESSED_KEY_EXCHANGES
    }
     ...
     */
    fun validatePacket(packet: IkoroPacket, peerID: String): Boolean {
        ...
        if (processedMessages.contains(messageID)) {
            ...
            val isFreshAnnounce = messageType == MessageType.ANNOUNCE &&
                    packet.ttl >= com.ikoro.android.util.AppConstants.MESSAGE_TTL_HOPS
     ...
     */
    private fun generateMessageID(packet: IkoroPacket, peerID: String): String {
     ...
     */
    private fun verifyPacketSignature(packet: IkoroPacket, peerID: String): Boolean {
        try {
            ...
            if (MessageType.fromValue(packet.type) == MessageType.ANNOUNCE) {
                ...
                try {
                    val announcement = com.ikoro.android.model.IdentityAnnouncement.decode(packet.payload)
