package com.ikoro.android.sync

import android.util.Log
import com.ikoro.android.mesh.BluetoothPacketBroadcaster
...
class GossipSyncManager(
    ...
    interface Delegate {
        ...
        fun sendPacketToPeer(peerID: String, packet: IkoroPacket)
        fun signPacketForBroadcast(packet: IkoroPacket): IkoroPacket
    }
    ...
    private val messages = LinkedHashMap<String, IkoroPacket>()
    // - announcements: only keep latest per sender peerID
    private val latestAnnouncementByPeer = ConcurrentHashMap<String, Pair<String, IkoroPacket>>()
    ...
    fun start() {
        ...
        cleanupJob = scope.launch(Dispatchers.IO) {
            while (isActive) {
                try {
                    delay(com.ikoro.android.util.AppConstants.Sync.CLEANUP_INTERVAL_MS)
    ...
    fun onPublicPacketSeen(packet: IkoroPacket) {
        ...
        if (isBroadcastMessage) {
            ...
            if (age > com.ikoro.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
                Log.d(TAG, "Ignoring stale ANNOUNCE (age=${age}ms > ${com.ikoro.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
                return
            }
    ...
    private fun sendRequestSync() {
        ...
        val packet = IkoroPacket(
            ...
            payload = payload,
            ttl = com.ikoro.android.util.AppConstants.SYNC_TTL_HOPS // neighbors only
        )
        // Sign and broadcast
    ...
    private fun sendRequestSyncToPeer(peerID: String) {
        ...
        val packet = IkoroPacket(
            ...
            timestamp = System.currentTimeMillis().toULong(),
            payload = payload,
            ttl = com.ikoro.android.util.AppConstants.SYNC_TTL_HOPS // neighbor only
        )
    ...
    fun handleRequestSync(fromPeerID: String, request: RequestSyncPacket) {
        ...
        for ((_, pair) in latestAnnouncementByPeer.entries) {
            ...
            if (!mightContain(idBytes)) {
                ...
                val toSend = pkt.copy(ttl = com.ikoro.android.util.AppConstants.SYNC_TTL_HOPS)
    ...
    private fun buildGcsPayload(): ByteArray {
        ...
        val list = ArrayList<IkoroPacket>()
        // announcements
