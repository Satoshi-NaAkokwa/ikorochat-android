...
/**
 * IkoroFilePacket: TLV-encoded file transfer payload for BLE mesh.
 * TLVs:
 *  - 0x01: filename (UTF-8)
 ...
 *
 * Note: The outer IkoroPacket uses version 2 (4-byte payload length), so this
...
data class IkoroFilePacket(
    ...
    fun encode(): ByteArray? {
        try {
            android.util.Log.d("IkoroFilePacket", "🔄 Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
                android.util.Log.e("IkoroFilePacket", "❌ TLV field too large: name=${nameBytes.size}, mime=${mimeBytes.size} (max: 65535)")
                return null
            }
            if (content.size > 0xFFFF) {
                android.util.Log.d("IkoroFilePacket", "📦 Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                android.util.Log.d("IkoroFilePacket", "📏 TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            android.util.Log.d("IkoroFilePacket", "✅ Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
            android.util.Log.e("IkoroFilePacket", "❌ Encoding failed: ${e.message}", e)
            return null
        }
    }

    companion object {
        fun decode(data: ByteArray): IkoroFilePacket? {
            android.util.Log.d("IkoroFilePacket", "🔄 Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                val result = IkoroFilePacket(n, s, m, c)
                android.util.Log.d("IkoroFilePacket", "✅ Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {
                android.util.Log.e("IkoroFilePacket", "❌ Decoding failed: ${e.message}", e)
                return null
            }
        }
    }
