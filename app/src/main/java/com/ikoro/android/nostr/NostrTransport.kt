...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.ikoro.android.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
    ...
    fun sendPrivateMessage(
        ...
        transportScope.launch {
            try {
                ...
                // Strict: lookup the recipient's current IkoroChat peer ID using favorites mapping
                val recipientPeerIDForEmbed = try {
                    com.ikoro.android.favorites.FavoritesPersistenceService.shared
                ...
                val embedded = NostrEmbeddedIkoroChat.encodePMForNostr(
                    content = content,
                    messageID = messageID,
    ...
    private fun sendNextReadAck() {
        ...
        transportScope.launch {
            try {
                ...
                val ack = NostrEmbeddedIkoroChat.encodeAckForNostr(
    ...
    fun sendDeliveryAckGeohash(
        ...
        transportScope.launch {
            try {
                ...
                val embedded = NostrEmbeddedIkoroChat.encodeAckForNostrNoRecipient(
                    type = NoisePayloadType.DELIVERED,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                )
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.ikoro.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.ikoro.android.geohash.ChannelID.Location) {
        ...
        transportScope.launch {
            try {
                ...
                // Build embedded IkoroChat packet without recipient peer ID
                val embedded = NostrEmbeddedIkoroChat.encodePMForNostrNoRecipient(
                    content = content,
                    messageID = messageID,
                    senderPeerID = senderPeerID
                ) ?: run {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.ikoro.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.ikoro.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.ikoro.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
