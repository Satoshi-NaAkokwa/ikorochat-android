...
import com.ikoro.android.model.PrivateMessagePacket
import com.ikoro.android.protocol.IkoroPacket
import com.ikoro.android.services.SeenMessageStore
import com.ikoro.android.ui.ChatState
import com.ikoro.android.ui.MeshDelegateHandler
import com.ikoro.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.ikoro.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("ikoro1:")) return@launch

                val base64Content = content.removePrefix("ikoro1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = IkoroPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.ikoro.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.ikoro.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = IkoroFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.ikoro.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = IkoroMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
