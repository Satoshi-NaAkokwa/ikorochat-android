...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.ikoro.android.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.ikoro.android.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    
    // MARK: - Public Message Management
    
    fun addMessage(message: IkoroMessage) {
        ...
        try { com.ikoro.android.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }

    // Log a system message into the main chat (visible to user)
    fun addSystemMessage(text: String) {
        val sys = IkoroMessage(
            sender = "system",
            content = text,
            timestamp = Date(),
            isRelay = false
    ...
    fun addChannelMessage(channel: String, message: IkoroMessage) {
        ...
        try { com.ikoro.android.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessage(peerID: String, message: IkoroMessage) {
        ...
        // Reflect into process-wide store
        try { com.ikoro.android.services.AppStateStore.addPrivateMessage(peerID, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessageNoUnread(peerID: String, message: IkoroMessage) {
        val currentPrivateChats = state.getPrivateChatsValue().toMutableMap()
        if (!currentPrivateChats.containsKey(peerID)) {
            currentPrivateChats[peerID] = mutableListOf()
        }
     ...
     */
    fun generateMessageKey(message: IkoroMessage): String {
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.ikoro.android.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
