...
object NotificationTextUtils {
     ...
     */
    fun buildPrivateMessagePreview(message: IkoroMessage): String {
        return try {
            when (message.type) {
                IkoroMessageType.Image -> "📷 sent an image"
                IkoroMessageType.Audio -> "🎤 sent a voice message"
                IkoroMessageType.File -> {
