...
import com.ikoro.android.ui.NotificationTextUtils
...
class MeshDelegateHandler(
                ...
                if (state.getJoinedChannelsValue().contains(message.channel)) {
                    ...
                    val viewingGeohash = try {
                        if (channel.startsWith("geo:")) {
                            ...
                            selected is com.ikoro.android.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
                        } else false
    ...
    override fun didUpdatePeerList(peers: List<String>) {
        coroutineScope.launch {
            ...
            runCatching { com.ikoro.android.services.MessageRouter.tryGetInstance()?.onPeersUpdated(peers) }
                ...
                if (isNostrAlias || isNoiseHex) {
                    ...
                    val canonical = com.ikoro.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                        ...
                        nostrPubHexForAlias = { alias ->
                            ...
                            if (com.ikoro.android.nostr.GeohashAliasRegistry.contains(alias)) {
                                com.ikoro.android.nostr.GeohashAliasRegistry.get(alias)
                            } else {
                                // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                                val prefix = alias.removePrefix("nostr_")
                                val favs = try { com.ikoro.android.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                                favs.firstNotNullOfOrNull { rel ->
                                    rel.peerNostrPublicKey?.let { s ->
                                        runCatching { com.ikoro.android.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                    ...
                    if (canonical != currentPeer) {
                        ...
                        com.ikoro.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                    ...
                    val favoriteRel = try {
                        ...
                        if (noiseKey != null) {
                            com.ikoro.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                        } else null
                    ...
                    if (favoriteRel?.isMutual == true) {
                        ...
                        if (noiseHex != currentPeer) {
                            com.ikoro.android.services.ConversationAliasResolver.unifyChatsIntoPeer(
                                state = state,
            ...
            peers.forEach { pid ->
                try {
                    ...
                    val npub = com.ikoro.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
                    val tempNostrKey: String? = try {
                        if (npub != null) {
                            val (hrp, data) = com.ikoro.android.nostr.Bech32.decode(npub)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.ikoro.android.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    private fun checkAndTriggerMeshMentionNotification(message: IkoroMessage) {
        try {
     ...
     */
    private fun sendReadReceiptIfFocused(message: IkoroMessage) {
     ...
     */
    fun getPeerInfo(peerID: String): com.ikoro.android.mesh.PeerInfo? {
