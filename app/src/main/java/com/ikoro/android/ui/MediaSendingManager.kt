...
import com.ikoro.android.model.IkoroFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.ikoro.android.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val filePacket = IkoroFilePacket(
                fileName = file.name,
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, IkoroMessageType.Audio)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, IkoroMessageType.Audio)
            }
        } catch (e: Exception) {
    ...
    fun sendImageNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, IkoroMessageType.Image)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, IkoroMessageType.Image)
            }
        } catch (e: Exception) {
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.ikoro.android.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
            ...
            val messageType = when {
                mimeType.lowercase().startsWith("image/") -> IkoroMessageType.Image
                mimeType.lowercase().startsWith("audio/") -> IkoroMessageType.Audio
                else -> IkoroMessageType.File
            }
    ...
    private fun sendPrivateFile(
        toPeerID: String,
        filePacket: IkoroFilePacket,
        filePath: String,
        messageType: IkoroMessageType
    ) {
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.ikoro.android.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.ikoro.android.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.ikoro.android.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.ikoro.android.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
