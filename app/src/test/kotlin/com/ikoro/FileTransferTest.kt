...
class FileTransferTest {
    ...
    fun `encode and decode file packet with all fields should preserve data`() {
        ...
        val originalPacket = IkoroFilePacket(
            fileName = "test.png",
            mimeType = "image/png",
            fileSize = 1024000,
        ...
        val decoded = IkoroFilePacket.decode(encoded!!)
    ...
    fun `replaceFilePathInContent should correctly format content markers for different file types`() {
        ...
        val imageMessage = IkoroMessage(
            id = "test1",
            sender = "alice",
            senderPeerID = "12345678",
            content = "/data/user/0/com.ikoro.android/files/images/photo.jpg",
            type = IkoroMessageType.Image,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val audioMessage = IkoroMessage(
            id = "test2",
            sender = "bob",
            senderPeerID = "87654321",
            content = "/data/user/0/com.ikoro.android/files/audio/voice.amr",
            type = IkoroMessageType.Audio,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val fileMessage = IkoroMessage(
            id = "test3",
            sender = "charlie",
            senderPeerID = "11223344",
            content = "/data/user/0/com.ikoro.android/files/documents/document.pdf",
            type = IkoroMessageType.File,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )
        ...
        result = result.replace(
            "/data/user/0/com.ikoro.android/files/images/photo.jpg",
            "[image] photo.jpg"
        )
